...
{
	$gboxhide_date = get_option('gbox_info_hdate');
	if (!empty($gboxhide_date)) {
		$clickhide = round((time() - strtotime($gboxhide_date)) / 24 / 60 / 60);
		if ($clickhide < 25) {
			return;
		}
	}
	...
	$message = __('<strong><span style="font-weight:700;color:#17A15C">Wow!! Good News!! Gallery Box Pro Lifetime Now only $24 </span> <br> Upgrade to Gallery Box PRO and unlock advanced features By upgrading to Gallery Box PRO you get access to advanced youtube video gallery, Gallery Filter and more advanced features.<span style="font-weight:700;color:#b90000"> Now 30% off for you!!</span></strong>', 'gallery-box');
	...
	printf('<div class="%1$s" style="padding:10px 15px 20px;background: aliceblue;"><p>%2$s</p><a target="_blank" class="button button-primary" href="%3$s" style="margin-right:10px;background:#c50000">' . __('Upgrade Now', 'gallery-box') . '</a><a target="_blank" class="button button-primary" href="%4$s">' . __('View details', 'gallery-box') . '</a><a style="margin-left:10px;cursor: pointer;" class="gbox-dismiss">' . __('No Maybe later', 'gallery-box') . '</a></div>', esc_attr($class), wp_kses_post($message), $url2, $url1);
}
...
function gbox_admin_notice_option()
{
	if (isset($_GET['ghide']) && $_GET['ghide'] == 1) {
		update_option('gbox_info_hdate', current_time('mysql'));
	}
}
