...
if ( ! class_exists( 'xblog_About' ) ) {
    ...
    return add_query_arg(
		...
		public function free_pro() {
			...
			if ( ! empty( $free_pro ) ) {
				if ( ! empty( $free_pro['free_theme_name'] ) && ! empty( $free_pro['pro_theme_name'] ) && ! empty( $free_pro['features'] ) && is_array( $free_pro['features'] ) ) {
					...
					echo esc_html ('X Blog Pro Live Preview', 'x-blog-color');
					echo '</a></td></tr>';
		...
		public function style_and_scripts( $hook_suffix ) {
			...
			wp_enqueue_style( 'about-page-css', get_stylesheet_directory_uri() . '/inc/about/css/about.min.css', array(),'2.3.1','all'  );
        ...
        'activating_string'        => __( 'Activating', 'x-blog-color' )
        )
    );
